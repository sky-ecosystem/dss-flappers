...
* `minHop` - Minimum allowed value for `Splitter.hop`. `hop` may be raised freely up to a hard safety cap of just under 5 years. A large enough `hop` makes the farm re-rate `rewardRate = leftover / hop` truncate to absolute `0`; once `0`, no further `set` can revive it (`leftover = remaining * rewardRate` then computes as `0` too) — only a governance `notifyRewardAmount` can. The cap also keeps `hop` well below `type(uint256).max` — the halt sentinel reserved for governance (see Halting below) — so a facilitator cannot use `set` to halt the engine and lock itself out.
